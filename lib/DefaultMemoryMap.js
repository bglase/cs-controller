...
/**
 * Convert a value to bitmapped fields for TargetB ram location
 * 
 * @param item an object from the memory map 
 * @param value the value that should be converted 
 * 
 * @returns {Number}
 */
function valueToTargetBflags( item, value ) {
	return {
		key: !(value & 0x01),
		charging: !(value & 0x02),
		indoor: !(value & 0x20),
 ...
 * @returns {Number}
 */
function valueToTargetCflags( item, value ) {
	return {
		reverse: !(value & 0x01),
		quickstop: !(value & 0x40),
		brakeRelease: !(value & 0x80),
	};
    
}

// Traverse an object and count the number of MapItem objects
function countItems( items ) {
  var total = 0;
  
  if(items instanceof MapItem)
      return 1;
  else {
      for( var node in items ) {
          if( items[node] instanceof MapItem )
              total++;
          
          if( 'object' == typeof( items[node]))
              total = total + countItems( items[node]);
      }
    }
  return total;
...
/** 
 * flatten an object into an array of MapItems
 * 
 * @param object items an object containing some number of MapItem instances.
 * 
 * @return array of MapItems found within the object
 */
function flattenMap( items ) {
  var list = [];
  ...
  if(items instanceof MapItem)
      ...
      for( var node in items ) {
          if( items[node] instanceof MapItem )
              list.push(items[node]);
          
          else if( 'object' == typeof( items[node]))
              list = list.concat( flattenMap( items[node]));
      }
    }
  return list;
}
...
function MapItem( eeAddr, ramAddr, options ) {
    this.eeAddr = eeAddr;
    this.ramAddr = ramAddr;
    this.value = options.value || 0;
    this.min = options.min || 0;
    this.max = options.max || FULL_SCALE;
    this.toFriendly = options.format || null;
    this.fromFriendly = options.unformat || null;
    this.name = options.name || 'ADDR' + eeAddr;
    this.units = options.units || '';
};

module.exports = { 
  
  // utility functions - this makes them accessible outside this module      
  fn: {
  ...
  config: {
      ...
      personality: {
        nominalBattery    : new MapItem( EE_NOMINAL_BATTERY, { min: 0x00,              max: 0xFF,              name: 'NOMINAL BATTERY' }), 
        firmwareRevHi     : new MapItem( EE_FIRMWARE_REV_HI, { min: 0x00,              max: 0xFF,              name: 'FW REV HI'       }), 
        firmwareRevLo     : new MapItem( EE_FIRMWARE_REV_LO, { min: 0x00,              max: 0xFF,              name: 'FW REV LO'       }), 
        ProductIdHi       : new MapItem( EE_PRODUCT_ID_HI,   { min: 0x00,              max: 0xFF,              name: 'PRODUCT ID HI'   }), 
        ProductIdLo       : new MapItem( EE_PRODUCT_ID_LO,   { min: 0x00,              max: 0xFF,              name: 'PRODUCT ID LO'   }), 
        serialNum03       : new MapItem( EE_SER_NUM_03,      { min: 0x00,              max: 0xFF,              name: 'SERIAL NUM 03'   }), 
        serialNum02       : new MapItem( EE_SER_NUM_02,      { min: 0x00,              max: 0xFF,              name: 'SERIAL NUM 02'   }), 
        serialNum01       : new MapItem( EE_SER_NUM_01,      { min: 0x00,              max: 0xFF,              name: 'SERIAL NUM 01'   }), 
        serialNum00       : new MapItem( EE_SER_NUM_00,      { min: 0x00,              max: 0xFF,              name: 'SERIAL NUM 00'   }), 
      ...
      */
              ...
              throttleValue      : new MapItem( EE_THROTTLE_VALUE, RAM_THROTTLE_VALUE, { min: 0x00, max: FULL_SCALE, format: valueToPercent, unformat: valueFromPercent, name: 'Throttle Value', units: '%' }), 
